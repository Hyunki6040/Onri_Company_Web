...
public class UtilFile {
	...
	public String fileUpload(MultipartHttpServletRequest request, MultipartFile uploadFile, String add_path, String add_file_name, String appli_birth) {
		String path = "";
		String fileName = "";	
		...
		try {
			...
			fileName = add_file_name.replaceAll("\\\\", "").replaceAll("/", "").replaceAll(":", "").replaceAll("\\*", "").replaceAll("\\?", "").replaceAll("\"", "").replaceAll("<", "").replaceAll(">", "").replaceAll("|", "");
			byte[] bytes = uploadFile.getBytes();
			path = getSaveLocation(request, add_path);

			// System.out.println("UtilFile fileUpload fileName : " + fileName);
			// System.out.println("UtilFile fileUpload uploadPath : " + path);

			File file = new File(path + fileName + "." + FilenameUtils.getExtension(uploadFile.getOriginalFilename()));

			// 파일명이 중복으로 존재할 경우
			if (fileName != null && !fileName.equals("")) {
				if (file.exists()) {
					// 파일명 앞에 업로드 시간 초단위로 붙여 파일명 중복을 방지
					fileName += "_" + appli_birth;
					file = new File(path + fileName + "." + FilenameUtils.getExtension(uploadFile.getOriginalFilename()));
				}else {
					file = new File(path + fileName + "." + FilenameUtils.getExtension(uploadFile.getOriginalFilename()));
				}
			}
		...
		return "upload_files/" + add_path + fileName;
	}

	// 업로드 파일 저장 경로 얻는 메소드

	// 업로드한 파일의 경로가 도메인 별로 달라야 했기 때문에 도메인의 형을 비교하여 파일 저장 정로를 다르게 지정함
	private String getSaveLocation(MultipartHttpServletRequest request, String add_path) {

		/*String uploadPath = request.getSession().getServletContext().getRealPath("/");*/
		String uploadPath = "/usr/local/";
		String attachPath = "upload_files/"+ add_path;

		//System.out.println("UtilFile getSaveLocation path : " + uploadPath + attachPath);
		
		File Folder = new File(uploadPath + attachPath);
